...
using System.ComponentModel.DataAnnotations; //add at the top
...
public class CheckoutModel : PageModel
{
    ...
    public CheckoutModel(BasedContext context)
    {
    ...
    public async Task OnGetAsync()
    {
        if (Request.Cookies[nameof(Basket)] is not null)
        {
           ...
           JsonSerializer.Deserialize<Basket>(Request.Cookies[nameof(Basket)]);
            ...
            {
                var selectedProducts = Basket.Items.Select(b =>
               b.ProductId).ToArray();
                SelectedProducts = await context.Products.Where(p =>
               selectedProducts.Contains(p.Id)).ToListAsync();
            }
        }
    }
        ...
        if (ModelState.IsValid && Request.Cookies[nameof(Basket)] is not null)
        {
           ...
           JsonSerializer.Deserialize<Basket>(Request.Cookies[nameof(Basket)]);
            if (basket is not null)
            {
                ...
                Confirmation = $@"<p>Your order for {basket.NumberOfItems} 
item{plural} has been received and is being processed:</p>
 <p>It will be sent to {ShippingAddress}. We will notify you when 
it has been despatched</p>";
                ...
                Response.Cookies.Append(nameof(Basket), string.Empty, new
               CookieOptions
